...
# --- Page Configuration ---
st.set_page_config(
    page_title="Golf Performance Dashboard", page_icon="⛳️", layout="wide"
)
...
if raw_df.empty:
    ...
    selected_courses = st.sidebar.multiselect(
        "Select Course(s)", courses, default=courses
    )
    ...
    if filter_type == "Most Recent Rounds" and not unique_rounds.empty:
        num_recent = st.sidebar.number_input(
            "Number of recent rounds",
            min_value=1,
            max_value=len(unique_rounds),
            value=min(3, len(unique_rounds)),
        )
        ...
        last_y = st.sidebar.number_input(
            "Last Y rounds to consider",
            min_value=1,
            max_value=len(unique_rounds),
            value=min(20, len(unique_rounds)),
        )
        best_x = st.sidebar.number_input(
            "Best X rounds to select",
            min_value=1,
            max_value=last_y,
            value=min(8, last_y),
        )
        ...
        # Calculate average stableford points and select the best (highest) rounds
    ...
    if final_df.empty:
        ...
        tab1, tab2, tab3, tab4, tab5 = st.tabs(
            ["Scoring", "Driving", "Approach", "Short Game", "Putting"]
        )
        ...
        with tab1:
            ...
            col1.metric(
                "Avg Points per Hole",
                f"{scoring_stats.get('avg_stableford_points', 0):.2f}",
            )
            col2.metric(
                "Avg Strokes (Par 4)",
                f"{scoring_stats.get('avg_strokes_par_4', 0):.2f}",
            )
            col3.metric(
                "Avg Strokes (Par 5)",
                f"{scoring_stats.get('avg_strokes_par_5', 0):.2f}",
            )

            st.plotly_chart(
                create_scoring_bar_chart(scoring_stats), use_container_width=True
            )
            ...
            with c1:
                st.plotly_chart(
                    create_score_distribution_chart(scoring_stats, 3),
                    use_container_width=True,
                )
            with c2:
                st.plotly_chart(
                    create_score_distribution_chart(scoring_stats, 4),
                    use_container_width=True,
                )
            with c3:
                st.plotly_chart(
                    create_score_distribution_chart(scoring_stats, 5),
                    use_container_width=True,
                )
        ...
        with tab2:
            ...
            col2.metric(
                "Ball in Play %",
                f"{driving_stats.get('ball_in_play_rate_pct', 0):.1f}%",
            )
            col3.metric(
                "Avg. Distance", f"{driving_stats.get('avg_drive_distance', 0):.1f} yds"
            )

            st.plotly_chart(
                create_driving_dispersion_chart(driving_stats), use_container_width=True
            )
            st.plotly_chart(
                create_distance_by_location_chart(driving_stats),
                use_container_width=True,
            )
        ...
        with tab3:
            ...
            col2.metric(
                "Handicap GIR %", f"{approach_stats.get('handicap_gir_pct', 0):.1f}%"
            )

            st.plotly_chart(
                create_approach_dispersion_chart(approach_stats),
                use_container_width=True,
            )
        ...
        with tab4:
            ...
            col1, col2, col3 = st.columns(3)
            col1.metric(
                "Scrambling %", f"{short_game_stats.get('scrambling_pct', 0):.1f}%"
            )
            col2.metric(
                "Handicap Scrambling %",
                f"{short_game_stats.get('handicap_scrambling_pct', 0):.1f}%",
            )
            col3.metric(
                "Short Game Score", f"{short_game_stats.get('short_game_score', 0):.2f}"
            )
        ...
        with tab5:
            ...
            col1.metric(
                "Avg Putts per Hole",
                f"{putting_stats.get('avg_putts_per_hole', 0):.2f}",
            )
            col2.metric(
                "3-Putt Avoidance %",
                f"{putting_stats.get('three_putt_avoidance_pct', 0):.1f}%",
            )
            col3.metric("1-Putt %", f"{putting_stats.get('1_putt_pct', 0):.1f}%")

            st.plotly_chart(
                create_putt_distribution_chart(putting_stats), use_container_width=True
            )

            st.subheader("Diagnostic Stats")
            c1, c2 = st.columns(2)
            c1.metric(
                "Avg Putts per GIR", f"{putting_stats.get('avg_putts_per_gir', 0):.2f}"
            )
            c2.metric(
                "Avg Putts per Missed GIR",
                f"{putting_stats.get('avg_putts_per_missed_gir', 0):.2f}",
            )
