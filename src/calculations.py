...
def calculate_scoring_stats(df: pd.DataFrame) -> Dict[str, Any]:
    ...
    for par_type in [3, 4, 5]:
        ...
        if not par_df.empty:
            ...
            stats[f'par_{par_type}_par_or_better_pct'] = (par_df['Score vs Par'] <= 0).sum() / total_holes * 100
            stats[f'par_{par_type}_bogey_pct'] = (par_df['Score vs Par'] == 1).sum() / total_holes * 100
            stats[f'par_{par_type}_double_bogey_plus_pct'] = (par_df['Score vs Par'] >= 2).sum() / total_holes * 100
    ...
    for par_type in [3, 4, 5]:
        ...
        if not par_df.empty:
            total_holes = len(par_df)
            stats[f'par_{par_type}_2_plus_points_pct'] = (par_df['Score'] >= 2).sum() / total_holes * 100
            stats[f'par_{par_type}_1_point_pct'] = (par_df['Score'] == 1).sum() / total_holes * 100
            stats[f'par_{par_type}_0_points_pct'] = (par_df['Score'] == 0).sum() / total_holes * 100
...
def calculate_driving_stats(df: pd.DataFrame) -> Dict[str, Any]:
    ...
    stats['ball_in_play_rate_pct'] = (~drive_df['Tee Shot Location'].str.startswith('OB', na=False)).sum() / total_drives * 100
    ...
    for par_type in [4, 5]:
        ...
        if not par_df.empty:
            ...
            par_stats['ball_in_play_rate_pct'] = (~par_df['Tee Shot Location'].str.startswith('OB', na=False)).sum() / total_par_drives * 100
...
def calculate_short_game_stats(df: pd.DataFrame) -> Dict[str, Any]:
    ...
    if not inside_100_df.empty:
        stats['avg_strokes_to_green_inside_100y'] = inside_100_df['Shots Inside 100y'].mean()
    ...
    if not missed_handicap_gir_df.empty:
        net_par_or_better = (missed_handicap_gir_df['Strokes'] - missed_handicap_gir_df['Shots'] <= missed_handicap_gir_df['Par']).sum()
        stats['handicap_scrambling_pct'] = (net_par_or_better / len(missed_handicap_gir_df)) * 100
...
def calculate_putting_stats(df: pd.DataFrame) -> Dict[str, Any]:
    ...
    if not missed_handicap_gir_df.empty:
        stats['avg_putts_per_missed_handicap_gir'] = missed_handicap_gir_df['Putts'].mean()
